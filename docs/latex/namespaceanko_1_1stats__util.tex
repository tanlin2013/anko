...
\begin{equation*} \mathcal{AIC} = n \log(\mathcal{RSS}/n) + 2p, \end{equation*} where $ \mathcal{RSS} $ is the residual sum of squares.
...
\begin{equation*} \mathcal{BIC} = n \log(\mathcal{RSS}/n) + p \log(n), \end{equation*} where $ \mathcal{RSS} $ is the residual sum of squares.
...
Compute the number of roots (zeros) for derivative of x. 

This is equivalent to solve

\begin{equation*} f(x) = \frac{df}{dx} = 0. \end{equation*}
...
\begin{equation*} y = intercept + slope \times x. \end{equation*}
